...
bool bmi270Begin(void)
{
    ...
    uint8_t readWriteLen = 32;
    uint16_t configSize = sizeof(bmi270_config_file);
    uint8_t remain = (uint8_t)(configSize % readWriteLen);

    uint8_t *configFile = bmi270_config_file;

    uint16_t balByte = configSize - balByte;

    InitI2C(SDA_PIN, SCL_PIN, 400000);


    readRegister(BMI270_ADDRESS, BMI270_CHIP_ID, chipID);
    ...
    writeRegister(BMI270_ADDRESS, PWR_CONF, 0x00); /* Disable advanced power save mode */

    delayMicroseconds(500); //datasheet PG.18 said sleep(>450us)

    writeRegister(BMI270_ADDRESS, INIT_CTRL, 0x00);   /* Disable loading of the configuration */

    for(index = 0; index < balByte; index += readWriteLen)
    {
        uploadFile((configFile + index), index, readWriteLen);
    }

    for(index = balByte; index < configSize; index += 2)
    {
        uploadFile((configFile + index), index, readWriteLen);
    }

    writeRegister(BMI270_ADDRESS, INIT_CTRL, 0x01);

    writeRegister(BMI270_ADDRESS, PWR_CONF, 0x01);
...
bool checkInitialization(void)
{
    ...
    readRegister(BMI270_ADDRESS, INTERNAL_STATUS, initStatus);

    if(((initStatus >> 0u) & 1u) == 1u)
    {
        return true;
    }
...
{
    writeRegister(BMI270_ADDRESS, PWR_CTRL, 0x04);
    writeRegister(BMI270_ADDRESS, ACC_CONF, 0x17);
    writeRegister(BMI270_ADDRESS, PWR_CONF, 0x03);

    readRegisters(BMI270_ADDRESS, DATA_8, buffer, 6);
}

void uploadFile(uint8_t *configData, uint16_t index, uint16_t writeLen)
{
    uint8_t addrArray[2] = {0};

    addrArray[0] = (uint8_t)((index / 2) & 0x0F);

    addrArray[1] = (uint8_t)((index / 2) >> 4);

    writeRegisters(BMI270_ADDRESS, INIT_ADDR_0, addrArray, 2);
    writeRegisters(BMI270_ADDRESS, INIT_DATA, configData, writeLen);
}
...
void setLowPowerACCMeasure(void)
{
    ...
    readRegister(BMI270_ADDRESS, PWR_CTRL, rslt);

    rslt |= (1u << 2u);
    writeRegister(BMI270_ADDRESS, PWR_CTRL, rslt);

    readRegister(BMI270_ADDRESS, ACC_CONF, rslt);

    //rslt &= ~(1u << 7u); //Power optimized mode, 100 Hz
    //rslt |= (0x06 << 0u); //25Hz
    rslt = 0x00;
    rslt = (dataRate100 << 0u);
    writeRegister(BMI270_ADDRESS, ACC_CONF, rslt);

    readRegister(BMI270_ADDRESS, PWR_CONF, rslt);

    rslt |= (1u << 0u);
    writeRegister(BMI270_ADDRESS, PWR_CONF, rslt);

    readRegister(BMI270_ADDRESS, INT_MAP_DATA, rslt);

    rslt |= (1u << 2u);
    writeRegister(BMI270_ADDRESS, INT_MAP_DATA, rslt);

    readRegister(BMI270_ADDRESS, INT1_IO_CTRL, rslt);

    rslt |= (1u << 1u) | (1u << 3u); //Output level active High, Output enabled, Ouput push pull mode
    writeRegister(BMI270_ADDRESS, INT1_IO_CTRL, rslt);
}
...
uint8_t readInterrupt(void)
{
    ...
    readRegister(BMI270_ADDRESS, INT_STATUS_1, rslt);
...
uint8_t readAcc_Range(void)
{
    ...
    readRegister(BMI270_ADDRESS, ACC_RANGE, rslt);
